...
class DijkstraPlanner(PlannerBase):
    """Grid-based Dijkstra path planner inheriting from PlannerBase."""
    ...
    def __init__(self, world_config, grid_config):
        ...
        print(f"[Dijkstra] Grid built: {self.cols}x{self.rows} cells "
    ...
    def _dijkstra_search(self, start, goal):
        """
        Core Dijkstra algorithm running on 8-connected grid.
        Returns: list of (col, row) cells from start to goal.
        """
        sc, sr = start
        gc, gr = goal
        
        # Priority queue: (g_score, col, row)
        open_heap = []
        ...
        while open_heap:
            g, col, row = heapq.heappop(open_heap)
            node = (col, row)
            ...
            for dc, dr, move_cost in NEIGHBOURS:
                ...
                if new_g < g_cost.get(neighbour, float("inf")):
                    ...
                    heapq.heappush(open_heap, (new_g, nc, nr))
    ...
    def _smooth_path_los(self, waypoints):
        ...
        print(f"[Dijkstra] Smoothed: {len(waypoints)} -> {len(smoothed)} waypoints")
        return smoothed
    ...
    def plan(self, start, goal, obstacles=None):
        ...
        cell_path = self._dijkstra_search((sc, sr), (gc, gr))
        
        if not cell_path:
            print("[Dijkstra] WARNING: No path found - falling back to straight line")
            self.compute_time_ms += (time.perf_counter() - start_t) * 1000.0
        ...
        if world_path:
            ...
            if self._is_free((gc_orig, gr_orig)):
                ...
                print(f"[Dijkstra] Goal {goal} is blocked/inflated. Keeping snapped target {world_path[-1]} to avoid collision.")
        ...
        print(f"[Dijkstra] Path found: {len(world_path)} waypoints | "
