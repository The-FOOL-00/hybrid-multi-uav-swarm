...
- **Anti-Stuck Sidestep Performance**: The drone encounters **{int(latest_run['replans']) - 1 if not pd.isna(latest_run.get('replans')) else 0} waypoint fallbacks** and triggers multiple sidesteps. This confirms that the anti-stuck layer is active and successfully navigates around narrow street corners.
